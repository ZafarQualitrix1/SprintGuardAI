...
// Same cross-module boundary as FetchExternalSprintQuery -- Requirement Intelligence depends on
// this via QueryBus rather than importing `integration`'s infrastructure directly.
export class FetchExternalIssueDetailQuery {
  constructor(
...
@QueryHandler(FetchExternalIssueDetailQuery)
export class FetchExternalIssueDetailHandler
  implements IQueryHandler<FetchExternalIssueDetailQuery, ExternalIssueDetailPayload>
{
  constructor(
  ...
  async execute(query: FetchExternalIssueDetailQuery): Promise<ExternalIssueDetailPayload> {
    ...
    return connector.fetchIssueDetail(query.externalId, credentials, connection.config);
  }
}
