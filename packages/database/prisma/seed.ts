...
const PERMISSIONS = [
  ...
  { key: 'release:publish', description: 'Publish release readiness reports and toggle release gates' },
  { key: 'release:configure', description: 'Configure release readiness scoring weights and deduction rules' },
...
const ROLES: Record<string, { name: string; permissions: string[] }> = {
  ...
  PROJECT_ADMIN: {
    ...
    permissions: [
      ...
      'automation:read', 'automation:write', 'release:read', 'release:publish', 'release:configure', 'integration:manage',
      'test:admin-unlock',
    ],
  },
  QA_LEAD: {
  ...
  const PROMPTS = [
    ...
    },
    {
      ...
      template: [
        ...
        'If deployment is blocked or needs PM approval, say so plainly and name the reason.',
        '',
        'Readiness score: {{readinessScore}}/100',
        'Release status: {{releaseStatus}} (risk category: {{riskCategory}}, deployment probability: {{deploymentProbability}}%)',
        'Requirement coverage: {{requirementCoveragePercent}}%',
        'Test case coverage (approved vs generated): {{testCaseCoveragePercent}}%',
        'Manual execution pass rate: {{manualPassRate}}%',
        'Automation execution pass rate: {{automationPassRate}}%',
        'Open defects: {{openBlockerCount}} Blocker, {{openCriticalCount}} Critical, {{openHighCount}} High, {{openMajorCount}} Major, {{openOtherCount}} Medium/Minor/Trivial',
        'Regression completed: {{regressionCompleted}}',
        'Deployment checklist complete: {{deploymentChecklistComplete}}',
        'Mandatory rule flags: {{mandatoryFlags}}',
        '',
        'Respond with ONLY valid JSON (no markdown fences, no commentary) matching exactly this shape:',
